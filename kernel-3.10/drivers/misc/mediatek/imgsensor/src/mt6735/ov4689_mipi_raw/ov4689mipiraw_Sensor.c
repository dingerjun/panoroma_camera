...
#define PFX "OV4689_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV4689_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
    ...
    .hs_video = {
        .pclk = 120000000,                //record different mode's pclk
        .linelength = 2568,                //record different mode's linelength
        .framelength = 1556,            //record different mode's framelength
        .startx = 0,                    //record different mode's startx of grabwindow
        .starty = 0,                    //record different mode's starty of grabwindow
        .grabwindow_width = 1280,        //0xA80//record different mode's width of grabwindow
        .grabwindow_height = 720,       //0X5F0 //record different mode's height of grabwindow
        /*     following for MIPIDataLowPwr2HighSpeedSettleDelayCount by different scenario    */
        .mipi_data_lp2hs_settle_dc = 20,//unit , ns
        /*     following for GetDefaultFramerateByScenario()    */
        .max_framerate = 300,    
...
static SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5] =
{
	 ...
	 { 2720, 1536,      8,              4,        2704, 1528,   2704, 1528,   8, 4,    2688, 1520,      0,    0, 2688, 1520}, // video
 	 { 2720, 1536,      8,              4,        2704, 1528,   2704, 1528,   8, 4,    2688, 1520,      0,    0, 1280,  720}, //hight speed video
 	 { 2720, 1536,      8,              4,        2704, 1528,   2704, 1528,   8, 4,    2688, 1520,      0,    0, 2688, 1520} // Preview
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x3638, 0x00);     // ADC & analog
	write_cmos_sensor(0x0100, 0x00);     // wake up from sleep
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
	    ...
	    printk(" nick 4689 getID *sensor_id=0x%x, 4689_id=0x%x \n ",*sensor_id, imgsensor_info.sensor_id);	
            if (*sensor_id == imgsensor_info.sensor_id) {
                printk("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            printk("NICK 4689 getID Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
                             ...
                             UINT8 *feature_para,UINT32 *feature_para_len)
{
    ...
    printk("feature control featureID=%d \n", feature_id);
...
UINT32 OV4689_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    ...
    printk("OV4689_MIPI_RAW_SensorInit \n");	
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
